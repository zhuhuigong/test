...
namespace DuiLib {
    ...
    CMarkupNode CMarkupNode::GetChild(LPCTSTR pstrName)
    {
        ...
        while (iPos != 0)
        {
            if (lstrcmp(m_pOwner->m_pstrXML + m_pOwner->m_pElements[iPos].iStart, pstrName) == 0)
            {
                return CMarkupNode(m_pOwner, iPos);
            }
    ...
    LPCTSTR CMarkupNode::GetAttributeValue(LPCTSTR pstrName)
    {
        ...
        {
            if (lstrcmp(m_pOwner->m_pstrXML + m_aAttributes[i].iName, pstrName) == 0)
    ...
    {
        Release();
        SIZE_T cchLen = lstrlen(pstrXML) + 1;
