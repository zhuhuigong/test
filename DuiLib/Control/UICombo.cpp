...
namespace DuiLib {
    ...
    LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        else if (uMsg == WM_LBUTTONUP)
        {
            ...
            if (pControl && lstrcmpi(pControl->GetClass(), _T("ScrollBarUI")) != 0)
    ...
    {
        if (lstrcmpi(pstrName, DUI_CTR_COMBO) == 0)
    ...
    {
        if (lstrcmpi(pstrName, _T("textpadding")) == 0)
        {
        ...
        else if (lstrcmpi(pstrName, _T("dropbox")) == 0)
        {
            SetDropBoxAttributeList(pstrValue);
        }
        else if (lstrcmpi(pstrName, _T("dropboxsize")) == 0)
        {
