...
{
    if (lstrcmpi(pstrName, _T("homepage")) == 0)
    {
        m_sHomePage = pstrValue;
    }
    else if (lstrcmpi(pstrName, _T("autonavi")) == 0)
    {
        m_bAutoNavi = (lstrcmpi(pstrValue, _T("true")) == 0);
    }
    else
    {
...
{
    if (lstrcmpi(pstrName, DUI_CTR_WEBBROWSER) == 0)
